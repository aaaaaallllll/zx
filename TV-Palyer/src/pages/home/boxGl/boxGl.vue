<template>
  <div id="box">
    <div class="title">
      <span>Boxs</span>
      <div class="search">
        <input type="text" placeholder="search for lD" v-model="search">
        <i class="iconfont" >&#xe645;</i>
      </div>
    </div>
    <div class="content">
...
<!--        &lt;!&ndash;        <p :class="{active:checkAll}" @click="checkAllShow">Select All</p>&ndash;&gt;-->
<!--        &lt;!&ndash;        <b class="iconfont" @click="toggerAlert">&#xe696;</b>&ndash;&gt;-->
<!--      </div>-->
      <transition-group
        name="bounce"
        ...
        <div class="box" v-for="(item,index) in xsList" :key="item.id" >
          ...
          <div class="box-left">
            <div class="box-left-top">
              <span style="left: 51px">Agent: {{item.id}}</span>
              <span style="left: 311px">Deviceld: {{item.deviceId}}</span>
              <span style="left: 729px">Agent: {{item.dailishangid}}</span>
              <span style="left: 913px">VOD password: {{item.vodpwd}}</span>
            </div>
            <div class="box-left-bottom">
              <p style="left: 51px">Latest: {{item.createtime}}
              </p>
              <p style="left: 421px">DueDate: {{item.dueDate}}</p>
          ...
          <div class="box-right" :class="{in:item.ck}">
            <!--            编辑-->
            <i class="iconfont" @click.stop="alterItem(item.id,item.createtime,item.modifyTime)">&#xe619;</i>
            <!--            删除-->
    ...
    <div class="redact" v-if="showAdd" :key="223466">
      ...
      <div class="redact-hang">
        ...
        <div class="redact-ge" style="margin-right: 108px">
          <span>ID：</span>
          <input type="text" style="width: 216px" v-model="dailishangid">
        </div>
        <div class="redact-ge">
          <span>DueDate：</span>
      ...
      <div class="redact-hang" style="display: flex">
        <div class="redact-ge" style="margin-right: 108px">
          <span>VodPwd：</span>
          <input type="text" v-model="vodpwd">
        </div>
        <div class="redact-ge" style="margin-right: 108px">
          <span>UpdateAllowed：</span>
          <div>
            <p :class="{active:upShow}" @click="upShow=!upShow">Yes</p>
            <p :class="{active:!upShow}" @click="upShow=!upShow">No</p>
...
<script>
  ...
  export default {
      ...
      xsList(){
        ...
        if(that.search==''){
          ...
          for(let i=0;i<list.length;i++){
            if(list[i].id.toString().indexOf(that.search)!=-1){
              list2.push(list[i])
            }
          }
          return list2
    ...
    methods:{
          ...
          $.ajax({
            ...
            success: function (data) {
              ...
              if (!data.errorDesc) {
                ...
                that.list[xIndex].yunxuupdate=that.upShow
                that.afterIndex=-1
                that.gbAdd()
      ...
      //更改
      alterItem(id,createtime,modifyTime){
        ...
        that.upShow=that.list[index].yunxuupdate
        that.showAdd=true
        that.afterIndex=index
        that.msg='Redact'
        // that.id=id
        that.createtime=createtime
        that.modifyTime=modifyTime
      },
      // 获取数据
      getCookie(key) {
            ...
            if (!data.errorDesc) {
              ...
              for(let i=0;i<list.length;i++){
                let son={
                  ...
                  yunxuupdate:list[i].yunxuupdate,
