<template>
    <div class="back"  :style="note">
      <div class="login-box">
        ...
        <div class="bd"></div>
        <input type="text" placeholder="Username or E-mail address" v-model="userId">
            ...
            $.ajax({
              ...
              success: function (data) {
                ...
                if (data.errorDesc) {
                  ...
                  that.setCookie('country',data.result.data.country , 1)
                  that.setCookie('createtime',data.result.data.createtime , 1)
                  that.setCookie('dailishangming',data.result.data.dailishangming , 1)
...
<style scoped>
  ...
  .login-box .sub{
    height:72px;
    background:linear-gradient(180deg,rgba(172,224,241,1) 0%,rgba(122,203,212,1) 100%);
    border-radius:4px;
    line-height: 72px;
    font-size: 34px;
    color: #FFF;
