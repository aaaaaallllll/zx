...
Page({
  ...
  call() {
    const that = this;
    const userId = wx.getStorageSync('userinfo').techId //当前登录用户的用户名
    let fuwu = that.data.fuwulist1;
    let list = [];
    let sc = [];
    for (let i in fuwu) {
      if (fuwu[i].num > 0) {
  ...
  // 全选按钮
  quanxuan() {
    let zt = this.data.quanxuan;
    let list = this.data.fuwulist1;
    for (let i in list) {
  ...
  srk(e) {
    let gjz = e.detail && e.detail.value;
    if (!gjz) {
      this.setData({
        fuwulist1: this.data.fuwulist
      })
    } else {
      let list = this.data.fuwulist1;
      let list1 = [];
      for (let i = 0; i < list.length; i++) {
        if (this.getSpell(list[i].fuwu).indexOf(this.getSpell(gjz)) != -1) {
          list1.push(list[i])
        }
      }
      this.setData({
  ...
  btnj(e) {
    let list = this.data.fuwulist1;
    console.log(e)
    let nums = e.currentTarget.dataset.j;
    if (list[nums].num > 0) {
      list[nums].num--;
    } else {
      wx.showToast({
  ...
  btnjj(e) {
    let list = this.data.fuwulist1;
    let nums = e.currentTarget.dataset.ja;
    list[nums].num++;
    if (list[nums].num > 0) {
      list[nums].ck = true
      this.setData({
  ...
  getusers() {
    const that = this;
    const companyCode = wx.getStorageSync('userinfo').companyCode //门店id
    const techId = wx.getStorageSync('userinfo').techId //当前登录用户的用户名
    console.log(techId)
