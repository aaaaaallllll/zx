...
Page({
  ...
  data: {
    ...
    shopTitles: ['男牌', '女牌', '儿童', '人数合计', '男牌占比', '女牌占比'],
    gradeData: [],
    total: [],
    shopNames: [],
    companyList: [],
  ...
  getList() {
    ...
    wx.request({
      ...
      data: {
        ...
        queryType: 'KeLiuYueBao',
        year: that.data.begin,
        userId: userId
      ...
      success(res) {
        ...
        if (res.data.success) {
          ...
          let qiuHes=[0,0,0,0,0,0]
          for(let i=0;i<data.length;i++){
            qiuHes[0]+=Math.floor(data[i]['男牌'])
            qiuHes[1] += Math.floor(data[i]['女牌'])
            qiuHes[2] += Math.floor(data[i]['儿童'])
            qiuHes[3] += Math.floor(data[i]['人数合计'])
            qiuHes[4] = (Math.floor((qiuHes[0] / qiuHes[3]) * 10000))/100+'%'
            qiuHes[5] = (Math.floor((qiuHes[1] / qiuHes[3]) * 10000)) / 100 + '%'
          }
          console.log(qiuHes[1] / qiuHes[3])
          that.setData({
            gradeData: data,
            total: qiuHes
          })
