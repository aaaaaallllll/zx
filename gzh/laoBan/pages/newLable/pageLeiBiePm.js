// pages/newLable/pageLeiBiePm.js
import {
  Datas
} from '../../utils/data.js'
import {
...
Page({
  ...
  data: {
    ...
    huiZhongType: '全部',
    huiZhongList: [],
    shopTitles: ['本日数量', '本日应收业绩', '本日实收业绩','排名'],
    gradeData: [{
      bumenName: '足疗部',
      leiBie: '一式',
      benRiShuLian: 5,
  ...
  getList() {
    ...
    let buMen = that.data.huiZhongType
    if (buMen=='全部'){
      buMen=''
    }
    wx.request({
      ...
      data: {
        ...
        queryType: 'XiangMuLeiBeiPaiMing',
        // begin: '2019-07-05',
        // end: '2019-07-06',
      ...
      success(res) {
        ...
        if (res.data.success) {
          ...
          for(let i=0;i<data.length;i++){
            ...
            qiuHes[0] += Math.floor(data[i].shuLiang)
            qiuHes[1] += Math.floor(data[i].yingShou)
            qiuHes[2] += Math.floor(data[i].shiShou)
          }
          
          let hash = [];
          for (let i = 0; i < list.length; i++) {
            if (hash.indexOf(list[i]) == -1) {
              hash.push(list[i]);
            }
          }
          ...
          data.sort((a,b)=>{
            return b.shiShou - a.shiShou
          })
          that.setData({
            gradeData: data,
            total: qiuHes,
            huiZhongList: list2,
            xuanz: that.data.huiZhongType
          })
        } else {
          wx.showToast({
  ...
  bindPickerHuiZhongtype(e) {
    var that = this;
    console.log('picker发送选择改变，携带值为', e.detail.value)
    that.setData({
      huiZhongType: that.data.huiZhongList[e.detail.value].name
    })
    console.log('1')
  },
  bindPickerMingXitype(e) {
    var that = this;
    console.log('picker发送选择改变，携带值为', e.detail.value)
    that.setData({
      mingXiType: that.data.mingXiList[e.detail.value].name
    })
    console.log('1')
  },
})