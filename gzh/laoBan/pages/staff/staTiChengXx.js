  ...
  getList() {
    ...
    wx.request({
      ...
      data: {
        ...
        queryType: 'YuanGongTiChengMingXi',
      ...
      success(res) {
        ...
        if (res.data.success) {
          ...
          for(let i=0;i<data.length;i++){
            qiuHe[0] += Math.floor(data[i].number)
            qiuHe[1] += Math.floor(data[i].yingShou)
            qiuHe[2] += Math.floor(data[i].money)
            qiuHe[3] += Math.floor(data[i].tiCheng)
          }
          that.setData({
            gradeData: data,
            total: qiuHe
