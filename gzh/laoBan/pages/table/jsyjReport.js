...
Page({
  ...
  data: {
    ...
    shopTitles: ['总钟数', '点钟', '加钟', '总业绩','业绩排名', '点钟率', '加钟率','客人平均消费', '技师服务人数', '服务转化率', '会员占比', '业绩指标', '完成率', '会员卡销售指标', '会员卡销售金额', '会员卡指标完成率'],
    gradeData: [],
    shopName: '',
    datetype: 'date',
    showZbTitle:false,
      ...
      success(res) {
        ...
        if (res.data.success) {
          ...
          for(let i = 0;i<list.length;i++){
            let num = list[i].yeShenDate
            let time = new Date(num);
            let zt=time.getFullYear() + "-" + `${parseInt(time.getMonth()) + 1}` + "-" + time.getDate()
            // dates.push(zt)
            list[i].yeShenDate = zt
          }
