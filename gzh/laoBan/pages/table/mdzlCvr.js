        ...
        success(res) {
          ...
          if (res.data.success) {
            ...
            for (let j = 0; j < list.length; j++) {
              list2.push(list[j].proCat)

              if (list[j].proName == '转化率' || list[j].proName == '会员占比') {

                list4.push(list[j].proVal * 100 )
                list3.list.push(list[j].proVal * 100 + '%')
              } else {
                list4.push(Math.floor(list[j].proVal))
                list3.list.push(Math.floor(list[j].proVal))
              }
            ...
            let tList2 = []
            for (let j = 0; j < list.length; j++) {
              let index = tList.indexOf(list[j].proCat)
              if (indexList.indexOf(index) == -1) {
                ...
                tList2[index].list.push(list[j].proName)
              }
            }
            let qiuHeList = []
