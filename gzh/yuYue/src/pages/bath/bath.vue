...
<template>
  <div id="app">
    ...
    <div class="xiyu">
      <input type="number" v-model="renShu"  placeholder="请输入洗浴人数"><div @click="savaData()">确定</div>
    </div>
    <div class='room' v-if="showBox==1">
      ...
      <!--        &lt;!&ndash;                <span>开始时间：{{item.qiZhongTime?item.qiZhongTime:'无'}}</span>&ndash;&gt;-->
      <!--        &lt;!&ndash;                <span >剩余时间：{{item.leftTime}}分钟</span>&ndash;&gt;-->
      <!--      </div>-->
    </div>
...
<script>
  ...
  export default {
    ...
    data(){
      return{
        showBox:1,
        // roomData:['单人','双人','3~5人','5~10人','10人以上'],
        renShu:null,
        selectRoomData:'',
        companyCode:'',
        techId:'',
    ...
    methods:{
      ...
      savaData(){
        const that=this
        if(!/(^[\-0-9][0-9]*(.[0-9]+)?)$/.test(that.renShu)){
          that.$layer.msg('请输入人数');
          return
        }else if(that.renShu==0){
          that.$layer.msg('人数不能为零');
          return
        }
        // console.log(/(^[\-0-9][0-9]*(.[0-9]+)?)$/.test('asfasda1'))
