...
Page({
      ...
      if (str.charCodeAt(i) >= 0x4e00) {
        ...
        if (p > -1 && p < 3755) {
          for (t = qswhSpell.length - 1; t > 0; t = t - 2)
            if (qswhSpell[t] <= p)
              break;
          if (t > 0) ret += qswhSpell[t - 1].substr(0, 1) + sp;
        }
      } else {
  ...
  getTextList(){
    ...
    if (text==''){
      ...
      for (let i = 0; i < list.length; i++) {
        let index = that.getSpell(list[i].name).indexOf(text)
        if (index == -1) {
          list[i].ck = true
        }else{
          list[i].ck = false
