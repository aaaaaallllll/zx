<<template>
  <div>
    <div class="header">
      ...
      餐饮预订
      <i class="iconfont lt" @click="back">&#xe7eb;</i>
    </div>
    <!-- 选择日期 -->
    ...
    <!--选菜-->
    <div v-if="status==1" class="dianCai">
      <div class="left-nav">
        <div class="nav-box " v-for="(item,index) in changList" @click="changIndex=index" :class="{on:changIndex==index}">{{item.nape}}</div>
      </div>
      <div class="right-nav"  v-for="(items,index) in changList" v-if="changIndex==index">
        <div class="centent" v-for="(item,index) in items.list" >
          <img :src="item.yunDuanImgUrl" >
          <div class="info">
            <h3>{{item.name}}</h3>
            <div class="box">
              <div  class="jiage">
             ...
             <div class="shulian">
               <img src="../../images/jian_1.png" v-if="item.number>0" @click="tgNumber(0,index)">
               <span>{{item.number}}</span>
               <img src="../../images/add_1.png"  @click="tgNumber(1,index)">
             </div>
            </div>
          </div>
        </div>
      ...
      <div class="gwc"  @click="checkedok(2)">
        <span >{{inList.length}}</span>
        <img src="../../images/cart.png">
      </div>

    </div>
    <!--预约信息-->
    <div v-if="status==2" class="main">
      <div class="roomInfo">
      ...
      <div class="right-nav">
        <div  v-for="(items,index) in changList">
          <div class="centent" v-for="(item,index) in items.list" v-if="item.number>0">
            <img :src="item.yunDuanImgUrl" >
            <div class="info">
              <h3>{{item.name}}</h3>
              <div class="box">
      ...
      <div class="m-txt">
        <div class="wp" style="color: red;text-align: right">
          合计：{{qiuHe}}
        </div>
      </div>
      <div class="m-txt">
    ...
    <div v-if="status==2" @click="showHyk=!showHyk" class="g-a1" >立即预约</div>
...
<script>
  ...
  export default {
    ...
    computed:{
      qiuHe(){
        ...
        for (let i=0;i<inList.length;i++){
          he=he+(inList[i].price)*(inList[i].number)
        }
        return he
      },
    },
    methods:{
      tgNumber(e,index){
        const that=this
        const n=that.changIndex
        let list=that.inList
        let bl=true
        const name=that.changList[n].list[index].name
        const id=that.changList[n].list[index].id

        if(e==0){
          that.changList[n].list[index].number--
          for(let i=0;i<list.length;i++){
            if(list[i].name==name){
              list[i].number=that.changList[n].list[index].number
              if( list[i].number==0){
                list.splice(i,1)
              }
            }
          ...
          // if(!that.changList[n].list[index].number){
          //   that.changList[n].list[index].number=0
          // }
          that.changList[n].list[index].number++
          let son={
            name:name,
            price:that.changList[n].list[index].price,
            yunDuanImgUrl:that.changList[n].list[index].yunDuanImgUrl,
            number:that.changList[n].list[index].number,
            id:that.changList[n].list[index].id
          }
      ...
      sub() {
        ...
        console.log(that.qiuHe)
        const inList=that.inList
        let list=[]
        for (let i=0;i<inList.length;i++){
          let son={
            menu:inList[i].name,
            number:inList[i].number
          }
          list.push(son)
        }
      ...
      getChangList(){
        ...
        $.ajax({
          ...
          data: {
            ...
            openid:that.openid,
            operationType:'queryLocal',
            queryType:'queryZiZhuMenus'
          },
          success: function (data) {

            let list=eval('('+data.result+')')
            let list2=eval('('+list.menus+')')
            let list3=[]
            // console.log(list2)
            for (var key in list2) {
              for(let i=0;i<list2[key].length;i++){
                list2[key][i].number=0
              }
              let son={
                nape:key,
                list:list2[key]
...
<style scoped>
  ...
  .right-nav .centent .info .box{

  }
  .right-nav .centent .jiage{
    height: 40px;
    float: left;
    padding: 0;
    margin: 0;
  ...
  .right-nav .centent .shulian{
    display: flex;
    height: 40px;
    float: right;
    line-height: 40px;
  }
  .right-nav .centent .shulian img{
    height: 30px;
    width: 30px;
    /*margin-bottom: 5px;*/
    margin: 5px;
  }
  .right-nav .centent  span{
    text-align: center;
    display: inline-block;
    line-height: 40px;
  }
  .right-nav .centent>img{
   display: inline-block;
    width: 90px;
    height: 90px;
    margin: 10px;
